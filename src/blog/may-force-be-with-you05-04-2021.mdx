...
Since that time, I've been hearing a lot about MDX files, which are essentially Markdown files that support JSX syntax. So I added in a Gatsby plugin for [MDX support](https://www.gatsbyjs.com/plugins/gatsby-plugin-mdx/?=mdx) and also a new syntax highlighter using [PrismJS React](https://github.com/FormidableLabs/prism-react-renderer#theming).  

🕹 (By the way, the next step is to allow some of this code to be editable using [React Live](https://github.com/FormidableLabs/react-live) which is pretty awesome -- so all of you can play around with the code here and see it change on the fly.  Kudos to [FormidableLabs](https://formidable.com/) for making such awesome packages!) 
